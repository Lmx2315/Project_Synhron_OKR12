...
BOOL CMy1986WSDDlg::OnInitDialog()
{
	...
	if(!GetBootCod())
	{
	    MessageBox("Ошибка открытия файла 1986ve1bootuart.HEX","File Error",
			MB_OK|MB_ICONSTOP);
		CDialog::OnCancel();
		return 0;  
	}
...
DWORD	dwadr;
	...
	while(nb == 1)
	{
		...
		if(nb != 1)
		{
			...
			ilboot = (i+16 - dwadrboot) & 0xfffffff8;
			return 1;
		}
		if(!GetDataHex())
		{
		...
		if(btype_hex == 0)
		{
			...
			if((dwadr<0x20100000)||((dwadr+bl_hex)>0x20103fff))
			{
				file.Close();
				return 0;
			}
...
BYTE ks;
	...
	txdbuf[4] = 0x00;
	com.WriteBlock(txdbuf,5);
	if((!com.ReadBlock(rxdbuf,1))||(rxdbuf[0]!=0x00))
	{
		str = "ошибка обмена";
		InsertStrToList();		
		com.Close();
			...
			for(k=0;k<8;k++)
			{
				if((BYTE)rxdbuf[k] != bufcod[k+(j<<3)+(i<<8)])
				{
					...
					str.Format("Verify failed adr=0x%08x dataw=0x%02x datar=0x%02x",
						k+(j<<3)+(i<<8),bufcod[k+(j<<3)+(i<<8)],(BYTE)rxdbuf[k]);
					InsertStrToList();
					com.Close();
					return 0;
				}
